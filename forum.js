...
d3.json("data/eco_2.json", function(links) {
...
//        link.target = nodes[link.target.name] || (nodes[link.target.name] = {name: link.target});
    ...
    var path = svg.append("g").selectAll("path")
      ...
      .enter().append("path")
        ...
        .style("stroke", function(d){return myColor(d.group) });

    var node = svg.append("g").selectAll("circle")
        .data(nNodes)
      .enter().append("circle")
        .attr("r", function(d){return (d.name.size +1) * 10})
    .call(d3.drag()
    ...
    var text = svg.append("g").selectAll("text")
      ...
      .enter().append("text")
        .attr("x", 5)
        .attr("y", ".31em")
        .style("font-size", function(d){return (d.name.size +1) * 14})
        .style("font-size", function(d){return (d.name.size +1) * 14})
